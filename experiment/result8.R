...
# Transposed adjacency matrix of PPI was used during PPI network diffusion which Markov Random Walk algorithm was used
...
#------------------------- RNAseq + Methyl + RPPA(diffused Pathway Graph(transposed adj mat was used)) -------------------------#
gmr <- list(g, m, r)
testStatistic <- c("DESeq2", "t-test", "t-test")
...
res_pa_tGMR_d_RF <- fit.classification(y=y, samples = samples, datapath = datapath, respath = respath, profile_name = profile_name,
                                      method = "DRW", pranking = "t-test", classifier = "rf",
                                      nFolds = 5, numTops=50, iter = 50)

save(res_pa_tGMR_d_RF, file=file.path('data/model/res_pa_tGMR_d_RF.RData'))

summary(res_pa_tGMR_d_RF)
print(res_pa_tGMR_d_RF$results)
print(res_pa_tGMR_d_RF$resample$Accuracy)

write.SigFeatures(res_fit=res_pa_tGMR_d_RF, profile_name=profile_name, method="DRW", respath=respath)


#------------------------- RNAseq + Methyl + RPPA(PPI Graph(transposed adj mat was used)) -------------------------#
testStatistic <- c("DESeq2", "t-test", "t-test")
...
res_pa_tGMP_RF <- fit.classification(y=y, samples = samples, datapath = datapath, respath = respath, profile_name = profile_name,
                                    method = "DRW", pranking = "t-test", classifier = "rf",
                                    nFolds = 5, numTops=50, iter = 50)


save(res_pa_tGMP_RF, file=file.path('data/model/res_pa_tGMP_RF.RData'))

summary(res_pa_tGMP_RF)
print(res_pa_tGMP_RF$results)
print(res_pa_tGMP_RF$resample$Accuracy)

write.SigFeatures(res_fit=res_pa_tGMP_RF, profile_name=profile_name, method="DRW", respath=respath)


# plot
xlabs <- c("GM", "GMR", "GMR_d", "GMP")
res_models <- list(res_pa_GM_RF, res_pa_GMR_RF, res_pa_tGMR_d_RF, res_pa_tGMP_RF)
