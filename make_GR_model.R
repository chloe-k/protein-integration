make_GR_model <- function(id, lim=NULL, type_used=NULL, prob=NULL, Gamma=NULL){
  # id - 18_28
  ...
  # model_name -> res_pa_GR_18_28.RData
  # id -> result18_28_GR
  result_name <- paste(c('result',id,'_GR'), collapse = '')
  ...
  fit.iDRWPClass(x=x, y=y, globalGraph=gr, testStatistic= testStatistic, profile_name = profile_name,
                 ...
                 id = result_name, prob = prob, Gamma = Gamma, pranking = "t-test", mode = "GR", AntiCorr=FALSE, DEBUG=TRUE)
  ...
  model_path <- paste(c('data/model/res_pa_GR_',id,'_LOOCV.RData'), collapse = '')
  # model_path <- paste(c('data/model/res_pa_GR_',id,'.RData'), collapse = '')
  
  name <- paste(c('res_pa_GR_', id, '_LOOCV'), collapse='')
  # name <- paste(c('res_pa_GR_', id), collapse='')
